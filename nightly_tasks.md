...
- Landed on origin: leftover product blobs from DIFF-309–317 (HomePage Open Chat + `/api` hypothesis form; guided/conversation/observation Chat next-steps; collector valid JS without `as any`; manifest `123`/`81` and Redis drop from current-runtime lists; POST_CUTOVER `/api` web row); product-smoke Open Chat + `/api` markers restored; this nightly log; WORKING/ui verifier notes; DIFF-318 record.
- Testing on this tree: rust-route-parity --check PASS; test-rust-route-parity PASS (4); post-cutover-runtime-audit PASS; ui-smoke PASS (53 files); check-chat-bounds PASS; validate-chat-script PASS; validate-media-script PASS; validate-panel-scripts PASS (23); normal-user-product-smoke --check PASS. cargo/clippy blocked on rustc 1.75 / lockfile v4 / edition2024. docker/Playwright live smokes not runnable here (`docker` missing).
