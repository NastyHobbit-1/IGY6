...
Static verifiers: `apps/web` `test:ui-smoke` / `test:panel-scripts` and `scripts/normal-user-product-smoke.sh --check` scan `apps/web/src` (including split components). `apps/web/src/app/page.tsx` is a two-line `HomePage` re-export. DIFF-315 verified the leftover `/api` + Open Chat + no-`as any` patches locally; origin still needs a full-file PUT for those product blobs.
