...
Static verifiers: `apps/web` `test:ui-smoke` / `test:panel-scripts` and `scripts/normal-user-product-smoke.sh --check` scan `apps/web/src` (including split components). `apps/web/src/app/page.tsx` is a two-line `HomePage` re-export. DIFF-316 landed source-trust Chat wording and verifier docs on origin. HomePage `/api` + Open Chat, guided/conversation/observation Chat next-steps, collector `as any` removal, manifest 123/81, and POST_CUTOVER Redis/`/api` wording were verified on a local grok worktree and still need a full-file PUT for those product blobs.
