...
Static verifiers: `apps/web` `test:ui-smoke` / `test:panel-scripts` and `scripts/normal-user-product-smoke.sh --check` scan `apps/web/src` (including split components). `apps/web/src/app/page.tsx` is a two-line `HomePage` re-export. DIFF-317 lands HomePage `/api` + Open Chat, guided/conversation/observation Chat next-steps, collector `as any` removal, manifest 123/81, POST_CUTOVER Redis/`/api` wording, and product-smoke checks for the Open Chat CTA and hypothesis `/api` form.
