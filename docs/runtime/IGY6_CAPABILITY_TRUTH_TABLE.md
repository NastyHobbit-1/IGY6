...
| CAP-019 | Media import backend | Real PDF/image/audio/video import and extraction | significantly advanced on grok | new_api_route + worker_runtime_behavior (media_file path) + artifact + evidence stubs | 5-6 (grok) | DIFF-237, 246 (grok) | write-api SourceType + gateway collection (now accepts media_file), artifacts (hash + mime + metadata), evidence pipeline (placeholder doc + evidence item for media) | /collection-runs/dry-run + manual-upload with media_file source_type | Postgres (sources, collection_runs, raw_artifacts with mime_type/size/metadata, normalized_documents with media note, evidence_items) | Full content-addressed storage for any media bytes; dry-run gives media-specific preview + warnings; ingest creates traceable run + artifact + evidence stub ("media registered") | - | SourceType + gateway collection exercised in tests | Backend now functional for safe media registration + provenance | Full deep extraction (OCR, vision description, audio transcript, PDF text) still deferred / external-tool friendly per specs and safety rules. Metadata + hash + review flow is real. | medium (registration + audit real and safe; extraction level explicit) | Enhance extraction in future grok commits or DIFF-248 style work |
