...
| CAP-018 | Browser/web/router collector | Real scoped collector for URL/browser/router data per collector contract | significantly advanced on grok | new_api_route + worker_runtime_behavior (generic + browser_export path) + tested foundations | 6-7 (grok) | DIFF-236, 246 (grok) | write-api SourceType (BrowserExport etc), gateway collection-runs/dry-run/manual-upload (now accept browser_export etc), collection_run + artifact + evidence pipeline | /collection-runs/dry-run, /collection-runs/manual-upload, sources + permissions | Postgres (sources, permissions, collection_runs, raw_artifacts, normalized_documents, evidence_items) | Generic + type-aware dry-run now returns richer previews for browser_export; ingest stores artifact + creates evidence items (basic parse for text exports) | - | SourceType tests + gateway collection paths exercised | Backend now functional for registration + dry-run + evidence creation using browser_export (user-provided exports) | Real import of provided browser export files + scoped preview/collect per contract; live browser/profile scraping still excluded | medium (foundations solid; deeper parsing can improve) | Continue on grok or later DIFFs for richer browser parsing + UI forms |
| CAP-019 | Media import backend | Real PDF/image/audio/video import and extraction | implemented on grok | worker_runtime_behavior + new_api_route + tested (classify/utf8 unit tests) | 6-7 (grok) | DIFF-237, 246, **268**, **269**, **270** | crates/igy6-media-extract, igy6-normalization (media bytes route), igy6-worker image tools, MediaImportMvp.tsx, constants MEDIA_IMPORT_TYPES | /sources (media_file), /collection-runs/manual-upload with content_base64 | Postgres (sources, collection_runs, raw_artifacts with mime/size, normalized_documents from extracted text, evidence_items) + artifact store binary | Binary upload stores original; worker normalization calls extract_text_from_media (pdftotext PDF text layer, tesseract image OCR, ffmpeg+whisper A/V transcript). Tools installed in worker image / host install helpers. | install.sh / install.ps1 media tool notes; worker Dockerfile tools | cargo test -p igy6-media-extract; workspace tests | Requires worker rebuild after install so tools are in container | Image-only PDFs with no text layer may yield empty extraction until page-render OCR is added; transcript/OCR quality depends on local engines and language packs | low-medium (registration + local extraction real; quality is local-tool limited) | Owner rebuild worker + smoke media upload; optional page-render OCR DIFF later |
...
| CAP-022 | Wi-Fi/RF signal intelligence | Ingest signal readings, map coverage, correlate outcomes | FULL ACCESS ON GROK | worker_runtime_behavior (system command capture + artifact + graph facts) | 7 (grok full) | grok branch | gateway full_access_collect (nmcli/iwlist + ps/env + walk) | /collection-runs/full-access | local artifacts + evidence + summary_json | System command stdout captured as artifacts; facts mined into graph_candidates | - | Compiles + exercised in full_access path | Yes on grok (local system snapshot) | Full RF mapping/correlation still basic but any accessible signal data is ingested and stored locally only | low (on grok branch) | Maintained on grok |
| CAP-023 | Stream recording/playback | Register streams, capture sessions, extract OCR/transcript/events | FULL ACCESS ON GROK (foundations) | worker_runtime_behavior (any accessible stream data as artifact + evidence) | 6 (grok) | grok | full_access collector + artifacts pipeline | /collection-runs/full-access (can target device files or urls) | artifacts + docs + evidence | Any readable stream-like data (files, /proc, urls) captured 100% locally | - | Full access path | Yes (as file/ url content) | Deep real-time OCR/transcript not wired but the capture and storage of anything accessible is active | low on grok | Enhanced on grok branch |
| CAP-024 | Image/visual generation | Generate diagrams/images and save artifacts | FULL ON GROK (external tool assisted possible) | script + artifact storage | 5 (grok) | grok | full access can ingest generated images from any local process or url | /collection-runs/full-access + artifacts | artifacts (any image bytes stored locally) | Any image the process can read/save is stored as artifact + evidence | - | Full access + artifacts | Yes (ingest path) | Product itself does not generate, but on grok it happily ingests any visual the user or local tools produce and keeps it 100% inside | low | User can pipe external gen into full collector on grok |
| CAP-025 | Code project analysis/patch workflow | Analyze repo, identify bugs, propose/apply approved patches, run tests | FULL ACCESS ON GROK | worker_runtime + full collector + evidence | 8 (grok) | grok | full_access_collect walks any readable code trees, creates artifacts + evidence + claims for every file | /collection-runs/full-access (scope: code dirs) | artifacts + normalized + evidence + graph candidates for "bug", "function", "dependency" etc. | Recursive read of any accessible source tree; mining for claims; system commands for build/test output captured | - | Full access exercised | Yes (local FS + command capture) | On grok the collector will happily read every .rs, .ts, Cargo.toml, etc. the process can see, store it, mine relationships, and keep every byte local. Patch application still via work items/approvals but the analysis is now extremely broad. | low (deliberate on grok) | This is the point of the grok branch per user request |
| CAP-026 | Capability truth table | Track claimed vs actual capability state across finished spec vs implementation | implemented (this DIFF) | docs + script_lifecycle (audit) + new_persistence (md) | 9 (this document) | DIFF-245 (planned), DIFF-246 (delivered), DIFF-270 (CAP-019 refresh) | docs/runtime/IGY6_CAPABILITY_TRUTH_TABLE.md (this file), audit package, specs.txt, DIFFs | n/a (docs + md) | docs (git tracked) | Manual + agent-driven audit process; can be re-run via commands in section 12 | normal-user-product-smoke, operator checks | The verification section of this DIFF + cargo/npm checks | This document + past smokes | Automation of table regeneration + diffing against prior | medium (was docs_only) | Maintain via future DIFFs; owner re-audit after WSL |
...
- Titles containing "generation", "self-improvement", "guardrails", "release readiness", "graph extraction", "collector" can be read as more complete than the scoped MVP or UI surfaces that were delivered (explicitly called out in DIFF-245 and individual DIFFs).
...
- **Media import (CAP-019):** binary registration, provenance, and local extraction (pdftotext / tesseract / ffmpeg+whisper) are real on grok as of DIFF-268. Do not claim cloud OCR/vision, perfect accuracy, or image-only PDF page-render OCR until those are added. Worker image must include tools.
- Graph/relationship is aspirational in README/infra + trails in answers; not full persisted review yet.

## 6. UI-only capabilities
Primarily the control surfaces from DIFF-235..239 before backend was added in later DIFFs. Current UI (normal-user tabs Chat/Data/Work/Settings/More) is mostly wired to real API for core paths. Advanced/More diagnostics and some cards for patterns/predictions/experiments still carry higher overclaim risk until live verification.

## 7. Docs-only capabilities
- Full collector contract steps for all source categories (specs section 2).
- Detailed wifi mapping, real-time stream OCR, camera/sensor fusion beyond full-access capture.
...
- Image-only PDF page-render OCR (text-layer PDF extraction is implemented).
...
- Media import + local extraction (CAP-019) via igy6-media-extract + worker tools (DIFF-268).
...
Limited in agent context (Codex sandbox often cannot run full Docker). Past owner smokes reported for ranges DIFF-219..223 and some later. Script-level (backup/restore/diagnostics/smoke --check) are fixture/script verified. Full live-stack + outcome-confirmed for advanced collectors and graph pending owner WSL on the pushed grok branch. Media extraction requires rebuilt worker image with tools present.

## 10. Planned/not-started finished-product capabilities
Full RF mapping/correlation beyond capture, real-time stream OCR beyond offline extract, product-native image generation, full graph persistence + review, experiment *execution*, autonomous method improvement, rich sensor fusion, complete provenance explanation surfaces for every reasoning step, production promotion readiness, image-only PDF page-render OCR.
...
- DIFF-248 Media Import Backend MVP (metadata + safe fallback; **extraction delivered later in DIFF-268**).
...
- Prohibited scope avoided: no main, no merges, no .env, no private data dump, no broad changes outside DIFF-270 scope, work only on grok.
- CAP-019 refreshed 2026-07-30 under DIFF-270 to match DIFF-268/269 media extraction reality.
- Ready for owner review and WSL verification on the pushed `grok` branch.

**End of IGY6_CAPABILITY_TRUTH_TABLE.md (CAP-019 refreshed under DIFF-270 on grok branch)**
